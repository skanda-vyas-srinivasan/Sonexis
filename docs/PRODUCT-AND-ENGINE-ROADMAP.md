...
For the next shipping milestone, use the [2.1.0 release roadmap](RELEASE-2.1.0-ROADMAP.md). This document retains the broader backlog and historical decisions.
...
**Tutorial polish revised — 2026-09-09:** Keep the build/edit/save lesson short. Exercise cards ask for concrete actions and the normal Next button does not bypass them; Left/Right navigation also allows reading through the lesson without performing those actions. Parameter edits complete on actual value changes. The wire-gain exercise keeps its slider open while editing and completes only when Done is clicked after a value change. **Settings is the explicit exception requested by the user:** explain Input Gain, Output Gain, and Ceiling, say no changes are needed, and invite experimentation later. Settings controls remain read-only during this explanation, with Next to continue.

Copy now starts with **Welcome to Sonexis** and “Follow along to learn how to use the app.” Exercise cards lead with a concrete action and add a short explanation where needed. Input Gain, Output Gain, and Ceiling each get a separate highlight and Next step. The opening settings card says “No changes needed” once; individual controls only explain their purpose. The settings strip overlays only the canvas below the tabs and graph toolbar; explanation cards sit below the strip. Opening settings does not resize the canvas.

Left/Right arrows navigate tutorial instructions in the main window. Left reviews visited cards without undoing audio or rerunning actions; Right returns through review or advances to the next card, including action steps, so users can read all three lessons without making changes. Continue/Finish choices remain explicit. The normal hands-on flow still advances when actions succeed. Review blocks workspace clicks; Right or Next moves forward without an additional return button. Arrow handling leaves focused controls, text fields, dialogs, and other windows alone.

After Ceiling, a “Your audio setup” card says: “Adjust these settings to suit your speakers, avoid clipping, and get the best listening experience.” Next continues to saving; no setting change is required.

On the first launch of each app version (`CFBundleShortVersionString`), Basics starts automatically after audio setup, including for users with restored chains. The version is recorded when the tutorial opens, so skipping or quitting does not replay it on every launch. Existing completion badges and workspace restoration are preserved. Each published version must increment the marketing version; rebuilding the same version does not retrigger onboarding. Basics and App chains end with Continue and Finish here. Continue offers the sequence Basics → App chains → Manual wiring, starting the next lesson only after the previous workspace restoration completes. Finish here exits immediately after restoration; Manual wiring ends with Finish. Each lesson is still available independently from Home → Tutorial.

The app-chain lesson uses real actions: add an app, add Bass Boost, open the menu bar, choose an existing preset, disable/re-enable that app’s effects, reopen its editor, and close its practice tab. The menu-bar prompt has an upward arrow. Instructions also appear inside the menu-bar panel, which scrolls to the practice app and outlines the requested control. Closing the practice tab highlights only that tab, keeps its title enabled, and brightens its close button. Advancement depends on successful operations on the correct chain. Tutorial state lives above individual canvas instances so switching tabs retains progress. The app lesson uses the existing preset library, including bundled starter presets, without a save exercise. If the user has deleted every preset, it skips preset selection and continues to the effects toggle. Lessons can finish independently or continue to the next part.

Practice state is excluded from persistence, including preset identity. The app lesson snapshots the complete workspace and restores the original chains, unsaved graphs, gains, selected tab, bypass, and running state on finish or exit. Failed restoration leaves the lesson active for retry and the saved workspace intact. Unrelated chain operations are blocked; the specific action requested by the lesson remains enabled. Basics and Manual wiring retain their canvas restoration behavior. Settings callouts stay clear of the panel and Next clicks bypass the outside-click monitor. Engine-start advancement requires a running engine. Regression checks: `Scripts/test-tutorial.sh` and tutorial isolation cases in `Scripts/test-chain-workspace.sh`. Full interactive/visual acceptance remains manual; the computer-use tool did not return app state during this pass.
...
Authorized scope: restore the last working canvas on relaunch, including unsaved changes, independently of named presets. First-launch redesign remains separate. Following the user's launch-screen preference, a recovered session starts on Home with its workspace queued; entering Build restores that canvas and preset identity. The original rule skipping automatic tutorials for recovered sessions is superseded by the per-version onboarding decision above. Capture and recording remain stopped; their running states are not serialized.
...
Active tutorials suspend workspace capture and flush the pending user workspace before demo edits begin. Skipping restores the prior canvas; a first-launch tutorial also has an explicit empty prior canvas. Tutorial completion now restores the original canvas and preset identity, just like exit; a preset explicitly saved during practice remains in the library. See the tutorial polish update below.
...
Undo and redo were removed during the polish pass. Once Sonexis gained independent live chains, app assignments, presets, gain settings, and graph edits, a global history made the scope of each action difficult to predict, while graph-only history appeared broken outside the canvas. The toolbar buttons, Command-Z handling, graph/workspace history storage, parameter gesture hooks, and associated tests were removed together. Autosave, workspace recovery, and confirmation for destructive chain removal remain. If history returns later, the UI should name the pending action and affected chain explicitly.

### Preset unlinking — 2026-09-10

Right-click the workspace preset name and choose **Unlink preset** to keep the current effects, wiring, and settings as an unnamed chain. The preset name and Modified status disappear; the next Save asks for a new name. The saved preset stays in the library, and the unnamed association persists with the workspace. This action is disabled during tutorials.
