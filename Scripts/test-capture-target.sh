...
TEST_DIR=$(mktemp -d "${TMPDIR:-/tmp}/sonexis-capture-target.XXXXXX")
trap 'rm -rf "$TEST_DIR"' EXIT
xcrun swiftc -module-cache-path "$TEST_DIR/module-cache" -I "$PRODUCTS" \
    "$ROOT_DIR/Tests/CaptureTarget/main.swift" "$BINARY_DIR/Sonexis.debug.dylib" -o "$TEST_DIR/target-tests"
